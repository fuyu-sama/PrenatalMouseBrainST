...
snn_path = Path.joinpath(
    Path.home(), f"workspace/mouse-brain-full/SCT/SNN/pattern/{idx}.csv")
coor_path = Path.joinpath(
    Path.home(),
    f"workspace/mouse-brain-full/spaceranger/{idx}/outs/spatial/coor-{idx}.csv")

he_image = Image.open(he_path)
cluster_df = pd.read_csv(snn_path, index_col=0, header=0)
...
fig.savefig(
    Path.joinpath(Path.home(),
                  f"workspace/mouse-brain-full/SCT/SNN/cluster/{idx}.jpg"))
plt.close(fig)
