...
for i in range(1, n_gene_clusters + 1):
    homer_path = Path.joinpath(
        WORKDIR,
        f"results/motifResults/gene-cluster/{idx}-{i}")
    processes = [
    ...
    read_pvalue.columns = [f"{idx}-{i}"]
    read_fraction.columns = [f"{idx}-{i}"]
...
vmax = fraction_df.max().max()
vmin = fraction_df.min().min()
for i in range(pvalue_df.shape[1]):
    for j in range(pvalue_df.shape[0]):
        sc = ax.scatter(
            i,
            j,
            c=fraction_df.iloc[j, i],
            s=pvalue_df.iloc[j, i],
            cmap="Reds",
            vmax=vmax,
            vmin=vmin,
        )
...
ax_legend.scatter(
    ...
    s=[
        pvalue_df.min().min(),
        (pvalue_df.min().min() + pvalue_df.max().max()) / 2,
        pvalue_df.max().max(),
    ],
)
...
ax_legend.set_yticklabels([
    "", f"{pvalue_df.min().min():.3f}",
    f"{(pvalue_df.min().min() + pvalue_df.max().max()) / 2:.3f}",
    f"{pvalue_df.max().max():.3f}", ""
])
ax_legend.set_title("-logP")
fig.savefig(
    Path.joinpath(
        WORKDIR,
        f"results/motifResults/gene-cluster/{idx}-go-2.jpg"),
    bbox_inches="tight",
)
plt.close(fig)
...
fig.savefig(
    Path.joinpath(
        WORKDIR,
        f"results/motifResults/gene-cluster/{idx}-go.jpg"),
    bbox_inches="tight",
)
plt.close(fig)
