...
sc3_path = Path.joinpath(
    WORKDIR,
    f"results/cluster/{scale_method}-sc3/pattern/sub-clusters/{idx}-hypothalamus-sc3.csv",
)
...
for i, j in zip(range(2, 6), ax.flatten()):
    j.axis("off")
    j.imshow(he_image)
    j.scatter(
        coor_df["X"],
...
fig.savefig(
    Path.joinpath(
        WORKDIR,
        f"results/cluster/{scale_method}-sc3/together/sub-clusters/{idx}-hypothalamus-sc3.jpg"
    ))
plt.close(fig)